#!/usr/bin/env python3
"""
日期起卦融合版 — 年月日→卦象 × 六层单卦师 × 世界线分化
信源: 日期(确定性, 但卦象结构提供世界线触碰机制)
"""
...
# ── 日期起卦 ──
def date_hex(y,m,d,obs_id=0):
    return ((y*7+m*13+d*3+obs_id*7919)%64, (y+m+d+obs_id)%6)
...
D=os.path.join(os.path.dirname(os.path.abspath(__file__)),'data_fusion')
...
da=np.full((N,3),0.5)
sp=os.path.join(D,'model_state.json')
if os.path.exists(sp):s=json.load(open(sp));wl=np.array(s.get('wl',wl.tolist()));da=np.array(s.get('display_acc',da.tolist()))

now=datetime.now();ts=now.strftime('%Y-%m-%d %H:%M');mo=now.month;y=now.year;dd=now.day

preds=[]
for oi in range(N):
    hi,dy=date_hex(y,mo,dd,oi);hi%=64
...
print(f"[{ts}] 日期版预测: {cn} (共识{vc[cs]}/5) | "+" ".join(["晴" if x==0 else "雨" for x in preds]))
json.dump({'time':ts,'prediction':cn,'preds':preds,'consensus':vc[cs]},open(os.path.join(D,'last_pred.json'),'w'))

vp=os.path.join(D,'verify_log.json')
if os.path.exists(os.path.join(D,'last_pred.json')):
    pv=json.load(open(os.path.join(D,'last_pred.json')))
    aw=get_weather();an="晴" if ic(aw) else "阴雨"
    pc=(1 if pv['prediction']=='晴' else 0)==(1 if ic(aw) else 0)
    for oi in range(N):
        hi,_=date_hex(y,mo,dd,oi);hi%=64
    ...
    vl.append({'time':ts,'prediction':pv['prediction'],'actual':an,'correct':pc})
    ...
    json.dump({'wl':wl.tolist(),'display_acc':da.tolist()},open(sp,'w'))
